...
| F-054 | **图片流 · 缩放与居中**：每条配图按 **实际宽高比** 计算行高（夹在 **最小 / 最大高度** 之间）；**`ContentScale.Fit`** 保证竖长图 **完整可见**（顶到最大高度时上下留白由暗底填充）；**`TransformOrigin.Center`**；**双指捏合**缩放（约 **1×～4×**）、放大后可 **单指拖动**；缩放时 **`nestedScroll`** 减少与外层列表抢竖滑；文字条仅在底部 **渐变+点击收起**。 | Done |
...
| F-056 | **图片流 · 双击缩放**：**1×** 时 **双击** 放大至约 **2×**（以触点为中心，位移经边界夹紧）；**已放大** 时 **双击** 恢复 **1×** 并清零位移，便于继续 **顺滑竖滑**；仍可用 **双指捏合**；无障碍文案区分两种双击。 | Done |
