...
| F-019 | **提醒 / 闹钟**：为单条微博设置 **本地通知**（`AlarmManager.setAlarmClock` + `Notification`）；详情 ⋮ **多久后提醒** 含 **3 分钟（验证）**、**15 分钟**快捷项，其余 **30 分钟一档**至 **24 小时**；**我** 页 **待处理提醒** 列表（到时前、可跳转微博、可取消）；点按通知进入微博后 **通知仍保留** 直至手动划掉；**logcat** 过滤 **`PW_Reminder`** 可跟调度与投递；无 **adb** 时可用 **设置 → 诊断日志**（**F-024**）导出内存日志；**DB 为准**：进程启动与 **开机 / 应用更新** 后从 `post_reminders` **重挂闹钟**，过期行 **立即补投递**；**Android 13+** 需通知权限；**不依赖外网**。 | Done |
