...
| F-008 | **配图体积控制**：超过约 1 MB 的待存图片使用 **zelory Compressor** 分档压缩（最长边 2048、目标约 1 MB，参见 **F-022**）；**发微博**界面可按本条开启 **原图（不压缩）**（仅影响本条中新添加的图片）。 | Done |
| F-009 | **配图体验**：写微博选图后在后台 **预先处理**（压缩/复制），发送时仅移动文件并 **防重复发送**（发送中/处理中禁用按钮与摇一摇）；详情页配图可 **全屏查看**（滑动换图、双指缩放；**双击** 打开/返回见 **F-021**）。 | Done |
...
| F-021 | **详情附图双击全屏 / 大图双击返回**：微博详情缩略图 **双击** 进入全屏查看；全屏内 **双击** 在已双指放大时先**还原缩放**，在 1× 时**关闭**全屏（仍保留关闭按钮与左右切图）。 | Done |
| F-022 | **配图压缩质量优先**：大图仍先走 **高分档 JPEG（最长边 2048）**；仅当体积仍超过目标约 **1 MB** 时再跑第二档并带软体积上限，减少「一刀切」糊图。 | Done |
