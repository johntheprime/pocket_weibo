...
| F-008 | **配图体积控制**：超过约 **1.5 MB** 的待存图片使用 **zelory Compressor** 分档压缩（最长边 2048、输出优先控制在约 **2 MB** 内，参见 **F-022**）；**发微博**界面可按本条开启 **原图（不压缩）**（仅影响本条中新添加的图片）。 | Done |
...
| F-021 | **详情附图预览手势（单击 / 双击）**：详情缩略图 **单击** 打开全屏预览；预览内 **双击** 在约 **1×** 与约 **2.5×** 间切换（已双指放大时双击先回到 **1×**）；预览内 **单击** 关闭（仍保留关闭按钮与左右切图、双指缩放）。 | Done |
| F-022 | **配图压缩质量优先**：大图先 **高质量 JPEG（最长边 2048、约 96 质量档）**；仅当仍超过约 **2 MB** 时再第二档（更高质量底 + 更缓的体积迭代）；**小于约 1.5 MB** 的选图尽量不重编码以保留细节。 | Done |
