...
| F-057 | **评论语音**：详情底栏与首页评论底栏支持 **麦克风录制**（最短时长等限制与同套录音组件一致）、**发送纯语音或文字+语音**；列表展示 **语音条播放**；发现评论搜索展示 **语音评论** 标签；ZIP 备份含 **`comment_attachments`**；删帖/删评/清空数据时清理语音文件。**写微博** 不提供语音录制入口（仅文字与配图）。 | Done |
