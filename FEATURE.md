...
| F-047 | **写微博 · 提醒替代艾特**：发微博页底栏第三枚由 **艾特** 改为 **提醒**（闹钟图标）；弹出与详情 **设置提醒** 一致的快捷时间与重复规则；选好时间后 **立即发布** 并为**新帖**写入 `post_reminders` 挂闹钟（含通知权限流程）。若当时 **不可发布**（无正文且未配图、未选身份、或仍在处理配图），则 **仅保存待应用提醒**，补全后需手动点「发送」。退出或返回关闭时清除未发送的待应用提醒。 | Done |
...
| F-053 | **原「消息」Tab → 图片流**：底栏 **图片**（`Image` 图标）；**仅展示带配图的微博**；**`HorizontalPager`** 全屏页（标题栏下 **整页视口**）**左右滑动** 切换，类似系统相册；**作者、时间、正文**叠在图底部渐变上；**点按图片**切换叠层显隐；无转发/提醒/评论入口。移除 **`MessageScreen` / `MessageViewModel`** 与旧评论消息列表。 | Done |
| F-054 | **图片流 · 缩放与居中**：每页 **铺满标题栏下区域**，**`ContentScale.Fit`** 整图 **水平垂直居中**（暗底填边）；**`TransformOrigin.Center`**；**双指捏合**缩放（约 **1×～4×**）、放大后可 **单指拖动**；**放大时暂停横向分页**（`userScrollEnabled`），避免与拖图冲突；页内 **`nestedScroll`** 与缩放协同。 | Done |
| F-055 | **详情 · 摇一摇设提醒**：微博详情页 **摇一摇** 为当前帖 **安排一次系统提醒**（检测比发帖页 **摇一摇发送** 更 **灵敏**：更短预热/冷却、更低加速度阈值、**两次**冲量即可）；默认 **今天 20:00**，若该时刻已过则 **次日 09:00**；成功 Toast **含提醒触发的绝对时间**（与 **`time_full_format`** 一致）；**设置** 可改 **晚间 / 早间**；复用与对话框 **提醒** 相同的权限与调度路径。 | Done |
| F-056 | **图片流 · 双击缩放**：**1×** 时 **双击** 放大至约 **2×**（以触点为中心，位移经边界夹紧）；**已放大** 时 **双击** 恢复 **1×** 并清零位移，便于 **左右滑动** 换图；仍可用 **双指捏合**；无障碍文案区分两种双击。 | Done |
| F-059 | **图片流 · 全屏分页相册**：图片 Tab 由 **固定高度卡片 + 纵向列表** 改为 **横向全屏分页**（`HorizontalPager`），当前照片在 **可用屏幕区域** 内 **居中展示**，交互贴近系统相册。 | Done |
| F-057 | **评论语音**：详情底栏与首页评论底栏支持 **麦克风录制**（最短时长等限制与同套录音组件一致）、**发送纯语音或文字+语音**；列表展示 **语音条播放**；发现评论搜索展示 **语音评论** 标签；ZIP 备份含 **`comment_attachments`**；删帖/删评/清空数据时清理语音文件。**写微博** 不提供语音录制入口（仅文字与配图）。**评论** 在 **输入框聚焦或已输入文字** 时 **隐藏麦克风**（空且未聚焦时显示；**录音中** 始终显示以便结束录制）。 | Done |
