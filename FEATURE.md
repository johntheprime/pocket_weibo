...
| F-027 | **我页与首页信息架构**：**我** 页顶部为简介与资料区；**身份管理** 迁入 **设置** 首项；**我的发布** 从 **我** 移除，改在 **首页** 标题栏中部入口；**待处理提醒** 在 **身份扩展资料**（国籍/职业/代表作等）**之后**、**我** 页靠下，列表限高可滚动；**重复提醒** 仅展示 **下一次** 与重复说明；**首页 / 发现列表** 微博卡片 **不展示配图缩略图**（详情仍可看图）。 | Done |
...
| F-034 | **首页 Tab 双击回顶**：在 **首页** 选中状态下 **快速双击** 底栏 **首页** 按钮，**滚动到列表顶部**（最新一条）并 **清除搜索筛选**、触发与下拉一致的 **刷新动效**（本地数据仍为 Flow 即时更新）。 | Done |
