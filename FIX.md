...
|-------------|---------------|------------|----------|
| 2026-05-08 | **图片 Tab**：照片困在 **固定高度卡片** 内，希望像 **系统相册** 整页居中 | **`PhotoScreen`**：以 **`HorizontalPager`** 替代 **`LazyColumn`**；每页 **`fillMaxSize`** + **`ContentScale.Fit`** 居中；**放大时** `userScrollEnabled=false` 避免与拖图冲突；切换页时重置分页锁定。更新 **F-053 / F-054 / F-056**，新增 **F-059**；**`photo_feed_reset_zoom_a11y`** 改为左右滑切换文案。版本 **3.56.0 (187)**。 | `./gradlew test assembleDebug` — BUILD SUCCESSFUL |
